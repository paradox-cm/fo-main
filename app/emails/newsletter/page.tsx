...
export default function NewsletterEmailPage() {
  const [copied, setCopied] = useState(false)

  const emailHtml = getNewsletterConfirmationEmailHtml("subscriber@example.com")
  ...
  return (
    <div className="container mx-auto py-12 px-4">
      <h1 className="text-3xl font-bold mb-8">Newsletter Subscription Email Template</h1>

      <div className="mb-8">
        <h2 className="text-xl font-semibold mb-4">Preview</h2>
        <div className="border rounded-lg p-6 bg-[#0F0F10]">
          <div dangerouslySetInnerHTML={{ __html: emailHtml.replace("{{email}}", "subscriber@example.com") }} />
        </div>
      </div>
